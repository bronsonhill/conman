...
`conman 0.1.0`, resolution model `0.4`. Run with built-in defaults (no
...
| firstmate | `420721401c4080d1a4f6982b0ef6769e2a749b23` | 1 | 18,299 | 0 | 1 | 0 | max-skills 1, per-file-budget 1, skill-index-budget 1, vehicle-fit 5 |
| motrix | `78610342d63ce8446acf4505158aaebdc91a9c09` | 38 | 92,356 | 0 | 0 | 0 | — |
| ack-nestjs-boilerplate | `ab70ad273db04566f1c03d9f797fba9f1e100738` | 2 | 7,000 | 0 | 0 | 0 | frontmatter 4, vehicle-fit 3 |
| cockroach | `8812064a015d2faf99d3fc7e15880f94042954b0` | 9 | 28,442 | 0 | 0 | 0 | frontmatter 9, max-skills 9, stale-boilerplate 9 |
| humanlayer | `99abe673498cf8bdcd5f989aebe9406a27185b3b` | 9 | 13,080 | 0 | 0 | 0 | stale-boilerplate 9 |
| posthog | `41570ae96afb3b0fd74d2873d68037553aaaec8d` | 52 | 1,352,459 | 0 | 52 | 0 | dead-reference 5, max-skills 52, per-file-budget 71, skill-index-budget 52, vehicle-fit 394 |
| ruflo | `d33ef4bf8ab27a8f9ef08352c9c293b53312a861` | 6 | 143,051 | 1,688 | 6 | 2 | dead-reference 12, duplication 30, max-skills 6, per-file-budget 8, skill-index-budget 6, stale-boilerplate 1, value-conflict 10, vehicle-fit 8 |
| lila | `9b49f37fe9d953c85dae12bbc159a0bf721a9fca` | 1 | 1,814 | 0 | 0 | 0 | — |
| inbox-zero | `780453c5d351a0b59b6ca2d7b60f6a58d213078b` | 2 | 5,566 | 0 | 0 | 0 | max-skills 2, vehicle-fit 6 |
| vercel-ai | `1c6854096fbe9aa55ae59cfa2bdee03d55e95c8e` | 2 | 7,918 | 0 | 0 | 0 | max-skills 2 |
...
- **Redundant tokens: 1,688 of 1,669,985, or 0.10%.** Byte-identical blocks that
...
- **Median resolved stack: 4,270 tokens.** Below the 10,800-token gate line, but
...
- **Over the effective budget: 59 of 123 entry points, or 47.97%.** Measured
  against the 10,800-token default gate line. The 59 are posthog (52), ruflo (6),
  and firstmate (1). Nothing in motrix, cockroach, humanlayer, llm, lila,
  ack-nestjs-boilerplate, inbox-zero, or vercel-ai crosses it.

## The boring results are the point

- **llm** resolves to a zero-file, zero-token stack. Its one `AGENTS.md` has no
  sibling `CLAUDE.md`, and at model 0.4 a bare `AGENTS.md` is discovered but not
...
adopters whose resolved stacks sit at ~5.6k and ~7.9k tokens, with only a
`max-skills` warning each — but two small repos do not offset a 52-entry-point
monorepo. Read the per-repo
...
budget. The corpus raises 716 findings in total; the rest break down as:

- **`vehicle-fit`** (416) — `warn`-level structural advice (skill-shaped content
  in a memory file, and similar), no gate effect. The most common finding by far,
  and mostly posthog's 394.
- **`per-file-budget`** (80) — a single resolved memory file over the
  4,000-token per-file cap. `warn`. `posthog` 71, `ruflo` 8, `firstmate` 1.
- **`max-skills`** (72) — a startup skill index over the count cap of 8.
  `posthog` 52, `cockroach` 9, `ruflo` 6, `inbox-zero` 2, `vercel-ai` 2,
  `firstmate` 1.
- **`skill-index-budget`** (59) — a startup skill listing over the 1,000-token
  cost cap. `warn`. `posthog` 52, `ruflo` 6, `firstmate` 1.
...
- **`dead-reference`** (17) — a memory file points at a path that does not exist.
  `ruflo` 12, `posthog` 5.
